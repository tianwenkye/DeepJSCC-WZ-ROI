...
import os
from models import DDJSCC_Grad_ablation
...
model.load_state_dict(torch.load('./JSCC_models/DDJSCC_grad_task_ablation_' + str(cr) + '_.pth.tar')['state_dict'])
    ...
    for k in range(0, 10):
        ...
        with torch.no_grad():
            for i, (x, y, gradcam, labels) in enumerate(test_loader):
                ...
                labels = labels.cuda()
                # gradcam_zeros = 0.25 + 0.25 * (torch.randn((labels.size(0), 1, 32, 32)))
