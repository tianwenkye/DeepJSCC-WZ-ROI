...
if CONTINUE_TRAINING == True:
    model.load_state_dict(torch.load('./JSCC_models/DDJSCC_gradcam_albation' + str(cr) + '_.pth.tar')['state_dict'])
    current_epoch = 0
for epoch in range(current_epoch, EPOCHS):
    ...
    with torch.no_grad():
        ...
        if averageacc > bestLoss:
            ...
            torch.save({'state_dict': model.state_dict(), }, './JSCC_models/DDJSCC_grad_task_ablation_'+str(cr)+'_.pth.tar')
            print('Model saved')
            bestLoss = averageacc